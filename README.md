...
### FAQ:
If you're having issues with installing tinycudann (likely because of CUDA toolkit mismatch):
1. `python -m pip install torch==2.1.2+cu121 torchvision==0.16.2+cu121 torchaudio==2.1.2+cu121 --index-url https://download.pytorch.org/whl/cu121`
2. Follow build steps from [source](https://github.com/NVlabs/tiny-cuda-nn?tab=readme-ov-file#compilation-windows--linux)
3. `cd bindings/torch`
4. `python setup.py install --user`
5. Reinstall torch version for nerfstudio `python -m pip install torchaudio==2.1.2+cu118 torchvision==0.16.2+cu118 --extra-index-url https://download.pytorch.org/whl/cu118`
6. For good measure, `pip install git+https://github.com/NVlabs/tiny-cuda-nn/#subdirectory=bindings/torch`